//"path" is a directory which contains folders "3,4,5,6...etc."
//Each subdirectories "3,4,5,6..." need to have the folder "XY" which contains reconstructed images
parameter = "1";
path = "R:/0_Information/from_Nasir-san/Experiment_3/";

//List of folders to work on "3,4,5,6...etc"
folderList = newArray(3,4,5,6,7);
...
for(folderIndex = 0; folderIndex<folderList.length;folderIndex++){
	...
	titleCut = "_"+folderList[folderIndex]+"_step1";
	//print(titleCut);
	for(fileListIndex=0;fileListIndex<currentFileList.length;fileListIndex++){
		if(indexOf(currentFileList[fileListIndex],titleCut)!=-1){
			fileExists = true;
			break;
		}
	}
	if(fileExists){
		print(titleCut+" file exists");
		continue;
	}
	//Directory Location
	path_in  = path+folderList[folderIndex]+"//XY";
	path_out = path;
	...
	//Open image and get parameters
	print("Opening XY images in folder: ["+folderList[folderIndex]+"]...");
	if(parameter=="0")
		continue;
	run("Image Sequence...", "open="+path_in+" sort");
	title = getTitle;
	newTitle = title+"_"+folderList[folderIndex]+"_step1_aligned_xy.tif";
	width = getWidth;
	height = getHeight;
	depth = nSlices;
	...
	run("Crop");
	print("Saving Translated/Cropped Image....");
	saveAs("Tiff", path_out + newTitle);
